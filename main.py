...
if __name__ == '__main__':
    ...
    backup_parser.add_argument('--compression', help=f"Type of compression ({", ".join(TAR_COMPRESSION_TYPES)}) to use on TAR file. Default is no compression.", type=str.lower, choices=TAR_COMPRESSION_TYPES, default=settings.DEFAULT_TAR_COMPRESSION_TYPE)
    backup_parser.add_argument('--encrypt', help=f"Specify to encrypt the TAR file. Key will be saved in state database. Nonce is TAR filename, repeated to {settings.ENCRYPT_NONCE_LENGTH} characters. Default is encryption enabled.", action=argparse.BooleanOptionalAction, default=True)
